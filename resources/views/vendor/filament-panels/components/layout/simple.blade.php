@php
    ...
    $bgPath = $settings->auth_background_path ?? null;
    $bgUrl = $bgPath ? asset('storage/' . $bgPath) : null;

    $cardOpacityValue = (int) ($settings->auth_card_opacity ?? 90);
    $cardOpacityValue = max(0, min(100, $cardOpacityValue));
    $cardOpacity = $cardOpacityValue / 100;

    // Tailwind slate-900 (#0f172a) with configurable alpha
    $cardBackgroundColor = "rgba(15, 23, 42, {$cardOpacity})";
@endphp
...
<x-filament-panels::layout.base :livewire="$livewire">
    ...
    <div
        class="fi-simple-layout flex min-h-screen flex-col items-center"
        @if ($bgUrl)
    ...
    >
        ...
        >
            <main
                @class([
                    'fi-simple-main my-16 w-full px-6 py-12 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/10 sm:rounded-xl sm:px-12',
